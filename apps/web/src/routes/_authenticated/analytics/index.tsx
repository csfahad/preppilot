...
function AnalyticsPage() {
    ...
    completed.forEach((i: any) => {
        (i.interviewTypes || []).forEach((type: string) => {
            ...
            typeScores[type].total += i.report?.overallScore || 0;
    ...
    const radarTotals: Record<string, { total: number; count: number }> = {};
    completed.forEach((i: any) => {
        if (i.report?.radarScores) {
            Object.entries(i.report.radarScores).forEach(([key, val]) => {
                if (!radarTotals[key])
                    radarTotals[key] = { total: 0, count: 0 };
                radarTotals[key].total += val as number;
                radarTotals[key].count += 1;
            });
        }
    });
    const radarData = Object.entries(radarTotals).map(([subject, data]) => ({
        subject,
                ...
                {completed.length === 0 ? (
                    <div className="bg-card border border-border rounded-2xl p-16 text-center">
                        ...
                        <p className="text-muted-foreground mb-6">
                            Complete at least one interview to see your
                            analytics
                        </p>
                        <Link
                            to="/interview/new"
